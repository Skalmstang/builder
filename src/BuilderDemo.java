...
class WordProcessor {
    ...
    private String secParagraph = "This should be a section paragraph";
    ...
    private String section2 = "Lets try one more header";
    private String subSection3 = "Suddenly it becomes more complex";
    private String paragraph3 = "Where should i go?";
 ...
 * document to keep the code small */
        ...
        builder.buildParagraph(secParagraph);
        ...
        builder.buildSection(section2);
        builder.buildSubsection(subSection3);
        builder.buildParagraph(paragraph3);
    }
}
...
class XMLBuilder implements  Builder {
    ...
    private String subsectionStart;
    private String subsectionEnd;
    ...
    @Override
    public void buildSection(String text) {
        start = sectionStart + subsectionStart + subsectionEnd + sectionEnd;
        sectionStart = "";
        subsectionStart = "";
        subsectionEnd = "";
        sectionEnd = "";
        indentation = 0;
        sectionStart += indentationString(indentation) + "<section name=''" + text + "''>\n";
        sectionEnd = indentationString(indentation) + "</section>\n" + sectionEnd;
    }

    @Override
    public void buildSubsection(String text) {
        sectionStart += subsectionStart + subsectionEnd;
        subsectionStart = "";
        subsectionEnd = "";
        indentation = 1;
        subsectionStart += indentationString(indentation) + "<subsection name=''" + text + "''>\n";
        subsectionEnd = indentationString(indentation) + "</subsection>\n";
    }

    @Override
    public void buildParagraph(String text) {
        indentation += 2;
        subsectionStart += indentationString(indentation)  +   "<paragraph>\n   " + indentationString(indentation) + text + "\n" + indentationString(indentation) + "</paragraph>\n";
        indentation -= 2;
    }

    public String getResult() {
        return start + sectionStart + subsectionStart + subsectionEnd + sectionEnd;
    }

    private String indentationString(int indentNum) {
        String result = "";
        while (indentNum > 0) {
            result += "   ";
            indentNum--;
